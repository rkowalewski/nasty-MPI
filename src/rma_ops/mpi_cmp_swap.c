...
static void _some_method(T_MpiRmaOp *me_super, int a, int b)
{
   ...
   T_MpiCmpSwap *me = mpi_cmp_swap__get_by_mpi_rma_op(me_super);
...
{
   memset(me, 0x00, sizeof(T_MpiCmpSwap));
...
{
   T_MpiCmpSwap *p_ret = NULL;

   if (instanceof_mpi_cmp_swap(me_super)){
...
{
   T_MpiCmpSwap *me = (T_MpiCmpSwap *)OOC_MALLOC( sizeof(T_MpiCmpSwap) );
   mpi_cmp_swap__ctor(me, p_params);
   return me;
}
